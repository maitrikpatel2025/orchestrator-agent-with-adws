...
# ///
"""
ADW Review Workflow - Standalone review step with worktree isolation and auto-PR.
...
4. Runs /review <prompt> <plan_path> agent inside the worktree
5. Extracts PASS/FAIL verdict from the review result
6. Pushes branch and creates PR for human review
7. Logs all events to agent_logs for swimlane visualization

Usage:
    uv run adws/adw_workflows/adw_review.py --adw-id <uuid>
"""
...
def load_command(command_name: str, variables: dict[str, str] | None = None) -> str:
    ...
    Args:
        command_name: Command filename (e.g., "plan.md", "review.md")
...
def create_logging_hooks(adw_id: str, adw_step: str, agent_id: str) -> HooksConfig:
    ...
    Args:
        ...
        adw_step: Current step slug (e.g., "review")
...
async def run_review_step(
    ...
    Args:
        ...
        user_prompt: The task prompt describing the work to review
        plan_path: Path to the plan file for review reference
    ...
    await log_step_start(
        ...
        adw_step=STEP_REVIEW,
        agent_id=agent_id,
        payload={"user_prompt": user_prompt, "plan_path": plan_path, "model": model},
        summary=f"Starting review step for: {user_prompt[:100]}...",
    )
    ...
    console.print(Panel(
        f"[bold yellow]Step 1/{TOTAL_STEPS}: Review[/bold yellow]\n\nPrompt: {user_prompt[:200]}...\nPlan: {plan_path}",
        title="ADW Review Workflow",
        width=console.width,
    ))
    ...
    try:
        ...
        if result.success:
            console.print(f"[green]Review step completed successfully (verdict: {verdict})[/green]")
            await log_step_end(
                adw_id=adw_id,
                adw_step=STEP_REVIEW,
                agent_id=agent_id,
                ...
                summary=f"Review step completed successfully (verdict: {verdict})",
            )
...
async def run_workflow(adw_id: str) -> bool:
    """Run the review workflow with worktree isolation and auto-PR.
    ...
    console.print(Panel(
        f"[bold]Starting ADW Review Workflow[/bold]\n\nADW ID: {adw_id}",
        title="ADW Workflow",
        width=console.width,
    ))
    ...
    review_model = input_data.get("review_model", model)
    ...
    try:
        ...
        # =================================================================
        # Step 1: Review (runs inside worktree)
        # =================================================================
        ...
        console.print(Panel(
            f"[bold green]Review Workflow Completed![/bold green]\n\n"
...
def main():
    """CLI entrypoint for the review workflow."""
    parser = argparse.ArgumentParser(
        description="ADW Review Workflow - Standalone review with worktree isolation and auto-PR",
